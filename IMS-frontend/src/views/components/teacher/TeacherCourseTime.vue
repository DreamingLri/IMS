...
const getCourseTimeList = () => {
  request.get("/course/listCourseWithTimeByUserId?userId="+user.id).then(res=>{
    if(res.code === 200){
...
const courses = ref([])
const getCourseList = () =>{
  request.get("course/listCourseByUserId?userId="+user.id).then(res => {
    if(res.code === 200){
  ...
  <div class="main-wrapper">
    <div style="height: 5%">
      <el-breadcrumb :separator-icon="ArrowRight">
        ...
        <el-breadcrumb-item>我的排课</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
