...
const submitForm = async (formEl) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      request.post('/student/insertUser' , addUserForm).then(res =>{
        if(res.code === 200){
...
const updateForm = async (formEl) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      request.post('/student/updateUserById', updateUserForm).then(res =>{
        if(res.code === 200){
...
const removeUser = (id) =>{
  request.delete("/student/deleteUserById/" + id).then(res => {
    if(res.code === 200){
